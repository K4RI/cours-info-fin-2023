...
class Student:
    def __init__(self, request, redis_db) -> None:
        ...
        # C'EST ICI LÀ IL Y AVAIT PAS LA MINUSCULE À BIRTH
        # ÇA FAISAIT TOUT PLANTER AAAAAAAH
        self.COURSE = "Course"
    ...
    def get_all_students(self):
        ...
        for id in ids:
            ...
            # ET LÀ AUSSI LES SELF.ID AU LIEU DE ID
            # ÇA RENVOYAIT TOUT LE TEMPS LE MÊME GRGRGRGRGRGR
            student[self.FIRST_NAME] = self.__hmget__(id, self.FIRST_NAME)[
                0
            ].decode("utf-8")
            student[self.LAST_NAME] = self.__hmget__(id, self.LAST_NAME)[0].decode(
                "utf-8"
            )
            student[self.BIRTHDAY] = self.__hmget__(id, self.BIRTHDAY)[0].decode(
                "utf-8"
            )
            student[self.COURSE] = self.__hmget__(id, self.COURSE)[0].decode(
                "utf-8"
            )
